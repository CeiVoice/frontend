...
const Side = ({ isOpen = true, onTrackingClick, onAdminClick }) => {
  return (
    <aside
    ...
    >
      <div className='h-full overflow-y-auto'>
        <div className='flex flex-col px-6 gap-6'>
          <button onClick={() => onTrackingClick?.()} className='select-none text-left bg-transparent text-black mt-8 font-bold hover:text-gray-400'>TRACKING</button>
          <button onClick={() => onAdminClick?.()} className='select-none text-left bg-transparent text-black hover:text-gray-400'>Admin</button>
